...
conda activate phylo-tp

# TOUT SUPPRIMER (sauf les données brutes et le script split)
rm -rf data/split/*
rm -rf results/
rm -rf logs/
rm -rf dag/
...
rm -rf .snakemake/
rm -f Snakefile
rm -f Snakefile.backup
rm -f Snakefile.old
rm -f test_aligned.fasta

# Vérifier qu'il reste seulement
ls -la
# Tu devrais voir :
# - data/raw/seqs-RNA.fasta
# - split_fasta.py
...
# =============================================================================
# Ce pipeline prend toutes les séquences et crée UN arbre phylogénétique
# montrant les relations évolutives entre les 59 espèces
# =============================================================================
...
echo "========================================="
echo "  EXÉCUTION AVEC 1 CŒUR"
echo "========================================="
echo ""

time snakemake --cores 1 --stats stats_1core.json -p 2>&1 | tee execution_1core.log
...
# 1. Voir le résumé
cat results/summary/analysis_summary.txt

# 2. Vérifier les fichiers créés
echo "=== Fichiers créés ==="
find results/ -type f -exec ls -lh {} \;

# 3. Voir l'arbre (format Newick)
echo "=== Arbre phylogénétique ==="
cat results/trees/phylogenetic_tree.treefile

# 4. Voir le DAG
xdg-open dag/workflow.png
...
echo "========================================="
echo "  NETTOYAGE POUR REFAIRE AVEC 4 CŒURS"
echo "========================================="
...
echo ""
echo "========================================="
echo "  EXÉCUTION AVEC 4 CŒURS"
echo "========================================="
echo ""

time snakemake --cores 4 --stats stats_4cores.json -p 2>&1 | tee execution_4cores.log
...
echo ""
echo "Temps avec 1 cœur:"
grep "^real" execution_1core.log
echo ""
echo "Temps avec 4 cœurs:"
grep "^real" execution_4cores.log
echo ""

# Calculer le speedup
echo "Calcul du speedup:"
echo "(Temps 1 cœur) / (Temps 4 cœurs)"
...
# Option 1 : Avec FigTree
figtree results/trees/phylogenetic_tree.treefile


-------------------------------------------------------------------------------------------------------------------

# Générer le DAG final
snakemake --dag | dot -Tpng > dag/workflow_final.png
snakemake --dag | dot -Tpdf > dag/workflow_final.pdf

# Voir le DAG
xdg-open dag/workflow_final.png
